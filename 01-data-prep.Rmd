...
hotel %>% janitor::tabyl(rating)

# Some reviews are as small as 16 characters, but they can get quite large.
hotel %>% pull(review_chrs) %>% summary()

# A few reviews
hotel %>% sample_n(3, seed = 12345) %>% select(review_chrs, review) %>% 
...
@Nagelkerke2020a recommends discarding reviews with few (<=50) tokens on the grounds that short reviews will not add much in terms of _different_ topics _within_ reviews.^[TODO: what does Nagelkerke mean - why is this important?]. That's about 25% of this sample. I'll leave them in for now.
...
bind_cols(
  review_id = 2:4,
  before = hotel %>% filter(review_id %in% 2:4) %>% pull(review),
...
# Example
bind_cols(
  hotel %>% filter(review_id == 3) %>% select(review),
...
```{r}
save(hotel, token, bigram, file = "input/hotel_prepped.Rdata")
```
